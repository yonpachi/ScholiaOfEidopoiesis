...
ルール本文は [`docs/`](../docs/)（PL 向け正本）を参照すること。開発用旧文書は [`legacy/`](../legacy/)。
...
| 層 | 正本 | 用途 |
| --- | --- | --- |
| `docs/` | ルール（PL 向け） | 公開ルールブック・正本 |
| `legacy/` | ルール（開発用旧文書） | 移行参照・非公開 |
| `catalog/` | ゲームデータ（本ディレクトリ） | 開発・シム参照・サイト掲載 |
| GitHub Pages | PL 提示用公開サイト | `docs/` + カタログを自動デプロイ |
